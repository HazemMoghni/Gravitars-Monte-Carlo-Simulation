...
a_ellipticity =  1.001000546 # Dimensionless normalizing constant
...
ellipticity_avg = 1e-6  # Dimensionless
...
def ellipticity(p):
    ...
    return root_scalar(cdf_ellipticity_p, bracket=[ellipticity_min, ellipticity_max * 1.001], args=(p,)).root
...
with open('output_GR_high.csv', 'a', newline='') as file:
    writer = csv.writer(file)
    if file.tell() == 0:
        writer.writerow(
            ['Radial Displacement (kpc)', 'Vertical Displacement (kpc)', 'Azimuthal Angle (rad)', 'Distance (kpc)', 'Ellipticity', 'Initial Period (s)',
             'Age (Yr)', 'Frequency (Hz)', 'log(f)', 'Strain', 'log(h)'])  # Write header row
    for _ in range(N):
        ...
        writer.writerow(
            [ri_output, zi_output, phi_output, d_output, ellipticity_output, Pi_output, age_output, f_GW_output, f_GW_output_log,
             h_0_output, h_0_output_log])
